...
CREATE TABLE public.aggregate_events_01_orders (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_orders_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    aggregate_state_id uuid,
                                                    event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_orders_event_and_return_id(
    IN event_in TEXT,
...
CREATE OR REPLACE FUNCTION insert_01_orders_aggregate_event_and_return_id(
    IN event_in TEXT,
    IN aggregate_id uuid, 
    in aggregate_state_id uuid
)
...
-- CREATE OR REPLACE PROCEDURE set_classic_optimistic_lock_01_orders() AS $$
-- BEGIN 
--     IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'aggregate_events_01_orders_aggregate_id_state_id_unique') THEN
-- ALTER TABLE aggregate_events_01_orders
--     ADD CONSTRAINT aggregate_events_01_orders_aggregate_id_state_id_unique UNIQUE (aggregate_state_id);
-- END IF;
-- END;
-- $$ LANGUAGE plpgsql;

-- CREATE OR REPLACE PROCEDURE un_set_classic_optimistic_lock_01_orders() AS $$
-- BEGIN
--     ALTER TABLE aggregate_events_01_orders
--     DROP CONSTRAINT IF EXISTS aggregate_events_01_orders_aggregate_id_state_id_unique; 
