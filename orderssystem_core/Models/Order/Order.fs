...
type Order private (id: Guid, userId: Guid, tableId: Guid, active: bool) =
    ...
    private new (id: Guid, userId, table) =
        Order (id, userId, table, true)

    new (userId: Guid, table: Guid) =
        Order (Guid.NewGuid(), userId, table, true)
    ...
    //         return Order (id, userId, (orderItem :: orderItems ) |> List.sort , tableId, true)
    //     }
    //
    ...
    //             
    //         return Order (id, userId, orderItems |> List.map (fun x -> if (sameOrderItem x) then orderItem else x), tableId, true)
    //     }
    ...
    //             |> Result.ofBool "OrderItem does not exist"
    //         return Order (id, userId, orderItems |> List.filter (fun x -> x.Id <> id), tableId, true)
    //     }
    //
    // you may want to deactiate it only if all order items are finished or aborted
    member this.Deactivate () =
        Order (id, userId, tableId, false) |> Ok
        // let res = 
        //     this.OrderItems 
        ...
        //     return Order (id, userId, tableId, false)
        // }
