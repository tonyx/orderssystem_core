...
open OrdersSystem.OrdersSystem // in the sharpino version
...
let authenticateUserWithSharpino (user: OrdersSystem.Models.User.User) =
    ...
    >=> sessionStore (fun store ->
        ...
        >=> store.set "temporary" user.Temporary
        >=> store.set "canmanagecourses" true
        >=> store.set "canmanageallorders" true)
    >=> returnPathOrHome
...
// FOCUS: posticipate THE SHARPINO VERSION
let logonViaQrCode  =
    // failwith "unimplemented"
    ...
    choose [
        ...
        POST >=> bindToForm Form.logon (fun form ->
            try
                ...
                let (Password password) = form.Password
                // match Db.Users.validateUser(form.Username, passHash password) ctx with
                match ordersSystem.AuthenticateUser (form.Username, passHash  password) with
                | Ok user ->
                    printf "re - entering in authentication\n"
                    authenticateUserWithSharpino user
                | _ ->
                    View.logon local.LoginOrPasswordInvalid |> html
            with
            | ex ->
...
let editIngredientCategory (idCategory: string) (user: UserLoggedOnSession) =
    ...
    let guidCategory = Guid.Parse idCategory // todo mind errors: tryParse
   
    let (converted, guidCategoryId) =  Guid.TryParse idCategory
    
    match converted with
    | false ->
        log.Error (sprintf "error parsing %s" idCategory)
        Redirection.FOUND Path.Errors.unableToCompleteOperation
    | true ->    
        // Redirection.FOUND Path.Errors.unableToCompleteOperation
        ...
        choose [
            GET >=> warbler (fun _ ->
                ...
                let allIngredientOfCategory = (ordersSystem.GetAllIngredientsOfACategory guidCategoryId).OkValue
                let category = (ordersSystem.GetIngredientType guidCategoryId).OkValue
            ...
            POST >=> bindToForm Form.ingredient
                (fun form ->
                    ...
                    if (ingredientFound.Length > 0) then
                        let allIngredientOfCategory = (ordersSystem.GetAllIngredientsOfACategory guidCategory).OkValue
